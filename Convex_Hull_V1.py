...
def segmentation_1(frame_energy):
    convex_hull[:] = []
    ...
    convex_hull.append(last_2)
    diff = [0] * len(convex_hull)
    for j in range(len(convex_hull)):
        diff[j] = convex_hull[j] - frame_energy[j]
        if diff[j] < 0:
            diff[j] *= -1

    if max(diff) > 0.3:
        st = frame_energy.index(frame_energy[0])
        bp = frame_energy.index(frame_energy[diff.index(max(diff))])
        sp = frame_energy.index(frame_energy[-1])

        if len(st_energy[st:bp]) > 0 and len(st_energy[bp:sp]) > 0:
            # plt.figure()
            # plt.plot(frame_energy)
            # plt.plot(convex_hull, 'red')
            # plt.vlines(diff.index(max(diff)), min(frame_energy), max(frame_energy))
            # plt.figure()
            # plt.plot(st_energy[st:bp])
            # plt.figure()
            # plt.plot(st_energy[bp:sp])
            # plt.show()
            ...
            print "h1"
            segmentation_1(frame_energy[bp:sp])
            print "b1"
            segmentation_1(frame_energy[st:bp])
        else:
            return ()
    else:
        return ()
...
def segmentation_2(frame_energy):
    convex_hull[:] = []
    break_point = frame_energy.index(max(frame_energy))
    last_1 = frame_energy[0]
    last_2 = frame_energy[break_point]
    ...
    if max(diff) > 0.3:
        ...
        if len(st_energy[st:bp]) > 0 and len(st_energy[bp:sp]) > 0:
            ...
            print "h1"
            segmentation_2(st_energy[bp:sp])
            print "b1"
            segmentation_2(st_energy[st:bp])
        else:
            return ()
    else:
        return ()
...
segmentation_1(st_energy)
segmentation_2(st_energy)

print segment_start
print segment_boundary
print segment_end

# print len(segment_boundary)
#
plt.plot(st_energy)
