"""
This script can be used to find the vowel candidates in an audio file using a peak elimination technique.

Input: A folder with the audio samples with the corresponding force aligned TextGrid file.

Output : A csv file storing the vowel count given by the algorithm, the vowel count given by the force aligned TextGrid,
the number of overlapping cases between the two (vowel count given by the algorithm and the vowel count given by the TextGrid),
the duration, the precision and the recall value for each audio file.
...
def peak_elimination(audiofile, window_dur=30, hop_dur=5, ripple_lower_limit=0.4, ripple_upper_limit=1.6, degree=3):
    try:
        ...
        data = np.concatenate((data, zero_array))  # Appending appropriate number of zeros
        length = len(data)  # Finding length of the actual data

        noise_energy = 0  # Initializing noise energy
        energy = []  # Initializing array to store energy
        for bit in range(length):
            energy.append(data[bit] * data[bit])  # Squaring each point of the data to calculate noise energy
        for ne in range(0, 800):
            noise_energy += energy[ne]  # Taking the first 800 samples of the original sound file
        noise_energy /= 800  # Averaging the square of the first 800 noise samples. To be used in threshold operation later
        ...
        if len(st_energy) < 300:
            st_energy = st_energy  # For shorter audio files, don't smooth the short term energy curve
        else:
            st_energy = savitzky_golay(st_energy, 51, degree)  # Smooth the short term energy curve
        ...
        new = ''
        new = new + (audiofile[0:76])
        new += '2\\'
        new = new + (audiofile[77:])
...
file_name_template_1 = 'F:\Projects\Active Projects\Project Intern_IITB\Vowel Evaluation PE V2\\'
result_name = 'Vowel_Evaluation_V2_test2.csv'
...
for j in only_audio:
    filename1, count1 = peak_elimination(j, window_dur=30, hop_dur=5, ripple_lower_limit=0.0, ripple_upper_limit=3.0, degree=5)  # Run Peak elimination algorithm
