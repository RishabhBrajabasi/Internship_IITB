...
def savitzky_golay(y, window_size, order, deriv=0, rate=1):
    ...
    first_value = y[0] - np.abs(y[1:half_window + 1][::-1] - y[0])
    last_value = y[-1] + np.abs(y[-half_window - 1:-1][::-1] - y[-1])
...
#---------------------------------------------------------------------------------------------------------------------#
def peak_elimination(audio, window_dur=30, hop_dur=5, degree=3, frames=20, threshold_smooth=120):
    new = ''
...
#---------------------------------------------------------------------------------------------------------------------#
        ...
        #     st_energy = st_energy
        # else:
        #     st_energy = moving_average(st_energy,frames)
#---------------------------------------------------------------------------------------------------------------------#
...
#----------------------------------------------------------------------------------------------------------------------#
        ...
        for p in range(no_frames):  # Finding the energy level of the peaks that have been chosen
            if p in loc:
                vowel_centre.append(st_energy[loc.index(p)])
            else:
                vowel_centre.append(0)
#----------------------------------------------------------------------------------------------------------------------#
        mark = []
        for p in range(len(vowel_centre)):  # Extracting a 50 ms slice of the audio file based on the frame number
            if vowel_centre[p] is not 0:
...
def textgrid(csvi):
    try:
        ...
        print 'Error in TextGrid creation', csvi
#----------------------------------------------------------------------------------------------------------------------#
...
def results_write(sr_no, name, vpe, vpee, vfa, t):
    if vpe == 0 or vfa == 0:
        ...
        results_vowel.write('\n')
#----------------------------------------------------------------------------------------------------------------------#
file_name_template_1 = 'F:\Projects\Active Projects\Project Intern_IITB\Vowel Evaluation PE V4\\'
result_name = 'Vowel_Evaluation_V4_allcorrect.csv'
...
#----------------------------------------------------------------------------------------------------------------------#
for j in only_audio:
    filename1, count1 = peak_elimination(j, window_dur=50, hop_dur=7, degree=10, frames=20, threshold_smooth=200)  # Run Peak elimination algorithm
    ...
    try:
        ...
        print 'Error in copying file:',str(filename1[:-4] + 'PE_NEW.TExtGrid')
    file_no += 1
