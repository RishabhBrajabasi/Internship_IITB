...
audio_file = 'F:\Projects\Active Projects\Project Intern_IITB\Vowel Evaluation PE V3\Analyze\Vowel_Evaluation_V3_I1\\254.wav'
...
ripple_value_thresh = []
count_of_vowels = 0
for k in range(len(ripple_value)-1):
    if 2.0 >= ripple_value[k] >= 0.4:
        ripple_value_thresh.append(ripple_value[k])
        count_of_vowels += 1
    elif ripple_value[k] > 2.0 and ripple_value[k+1] < 0.4:
        ripple_value_thresh.append(ripple_value[k])
        count_of_vowels += 1
    elif ripple_value[k] < 0.4 and ripple_value[k+1] > 2.0:
        ripple_value_thresh.append(ripple_value[k])
        count_of_vowels += 1
    else:
