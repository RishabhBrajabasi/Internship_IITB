...
#----------------------------------------------------------------------------------------------------------------------#
def peak_elimination(audio, window_dur=50, hop_dur=7, degree=5,ripple_lower_limit=0.4,ripple_upper_limit=2.0, threshold_smooth=100):
    new = ''
    new = new + (audio[0:76])
    new += '2\\'
    new = new + (audio[77:])
    ...
    #                 count_of_vowels += 1
    #             elif ripple_value[k] >= ripple_upper_limit and ripple_value[k+1] <= ripple_lower_limit :
    #                 ripple_value_thresh.append(ripple_value[k])
    #                 count_of_vowels += 1
...
result_name = 'Vowel_Evaluation_V3_Baseline.csv'

copyfile('C:\Users\Mahe\PycharmProjects\untitled\IIT-B Internship\\Test.py',
...
for j in only_audio:
    filename1, count1 = peak_elimination(j, window_dur=50, hop_dur=7,degree=7,ripple_lower_limit=0.4, ripple_upper_limit=2.0, threshold_smooth=100)  # Run Peak elimination algorithm
