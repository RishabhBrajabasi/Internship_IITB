...
def peak_elimination(audiofile, window_dur=30, hop_dur=5, degree=3, ripple_lower_limit=0.4, ripple_upper_limit=1.6, threshold_smooth=200, pr=20):
    try:
        ...
        #         if ripple_value[k] > 3.0 and ripple_value[k + 1] < 1.4 or ripple_value[k] > 1.02 and ripple_value[
        #                     k + 1] < \
        #                 0.3:
        ...
        return new, count_of_peaks_threshold
...
rll = [0]
ranger = [0]
cases = len(wd)*len(hd)*len(deg)*len(ts)*len(rul)*len(rll)*len(ranger)


results_opt = open('F:\Projects\Active Projects\Project Intern_IITB\Parameter Optimization PE V2\Vowel_opt_Window and Hop Test.csv', 'a')
if os.path.getsize('F:\Projects\Active Projects\Project Intern_IITB\Parameter Optimization PE V2\Vowel_opt_Window and Hop Test.csv') == 0:
    results_opt.write('File Name' + ',' + 'Window Duration' + ',' + 'Hop Duration' + ',' + 'Smooth threshold' + ','
                      + 'Ripple Lower limit' + ',' + 'Ripple Upper Limit' + ',' + 'Degree' + ',' + 'Precision' + ',' +
                      'Recall' + ',' + 'Ranger' + '\n')

file_name_template_1 = 'F:\Projects\Active Projects\Project Intern_IITB\Parameter Optimization PE V2\\'
                ...
                for rull in rul:
                    for de in deg:
                        for rang in ranger:
                            ...
                            for j in only_audio:
                                filename1, count1 = peak_elimination(j, window_dur=wdd, hop_dur=hdd, degree=de, ripple_lower_limit=rllo, ripple_upper_limit=rull, threshold_smooth=tss, pr=rang)  # Run Peak elimination algorithm
                                ...
                                file_no += 1
                            results_opt.write(str(result_name[file_count]) + ',' + str(wdd) + ',' + str(hdd) + ',' + str(tss) + ',' + str(rllo) + ',' + str(rull) + ',' + str(de) + ',' + str(rang) + ',' + str(sum(precision)/len(precision)) + ',' + str(sum(recall)/len(recall)) + '\n')
                            precision[:] = []
                            recall[:] = []
                            file_count += 1
                            file_no = 1
