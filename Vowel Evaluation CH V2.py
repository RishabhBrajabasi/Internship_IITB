...
def convexhull(audiofile, window_dur=50, hop_dur=5, threshold=0.3, threshold_smooth=200):
    try:
        ...
        text_file_1 = open(str(new[:-4]) + 'CH.csv', 'w')  # Opening CSV file to store results and to create TextGrid
...
file_name_template_1 = 'F:\Projects\Active Projects\Project Intern_IITB\Vowel Evaluation CH V2\\'
result_name = 'Vowel_Evaluation_V2_I5.csv'
...
for j in only_audio:
    filename2, count2 = convexhull(j, window_dur=50, hop_dur=7, threshold=0.2, threshold_smooth=120)  # Run the convex hull algorithm
    ...
    copyfile(str(filename2[:-4] + 'CH_NEW.TExtGrid'), file_name_template_1 + "Analyze\\" + result_name[:-4] + "\\" + str(file_no) + 'PE.TextGrid')
    file_no += 1
