...
#----------------------------------------------------------------------------------------------------------------------#
    ...
    for p in range(len(st_energy_peak)):
        if p == 0:
            ...
            if st_energy_peak[p] > st_energy_peak[p + 1] and st_energy_peak[p] > st_energy_peak[p - 1] and \
                ...
                peak_f.append(st_energy_peak[p])  # Finding value of the peak.
                location_peak_f.append(p)  # Finding location of the peak.
            else:
                peak_f.append(0)
    # ----------------------------------------------------------------------------------------------------------------------#
    # print peak_f
    # print location_peak_f
    the_list = [[]]
    the_list_value = [[]]
    count = 0
    for lines in range(len(location_peak_f)):
        value = d1[location_peak_f[lines]] * 0.85
        # print location_peak_f[lines]
        # print value
        # print range(location_peak_f[lines], -1, -1)
        flash_1 = location_peak_f[lines]
        flash_2 = location_peak_f[lines]
    ...
    for cc in range(len(superman)):
        cyborg.append(
            [valley[cc], superman[cc][0], superman[cc][1], superman[cc][2], valley[cc + 1], fresh_count])
        fresh_count += 1
    cyborg.pop(0)
    ...
    #     if i % 2 == 0:
    #         text_file_1.write(
    #             '%06.3f' % (better_boundaries[i][0]) + "\t" + '%06.3f' % (
    #             better_boundaries[i][1]) + "\t" + " " + "\n")
    #     else:
...
#----------------------------------------------------------------------------------------------------------------------#
    ...
    vowel = 0
    # print boundary
    for p in boundary:  # Extracting a 50 ms slice of the audio file based on the frame number
        mark.append(p[0] * hop_size)
        mark.append(p[1] * hop_size + window_size)
        vowel += 1
...
result_name = 'Vowel_Evaluation_V7_Test9.csv'
