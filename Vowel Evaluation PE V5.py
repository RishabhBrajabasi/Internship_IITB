...
def peak_elimination(audio, window_dur=30, hop_dur=5, degree=3, threshold_smooth=120):
    ...
    try:
        ...
        # st_energy = movingaverage(st_energy,20)
        ...
        for k in range(len(ripple_value)):
            if k != len(ripple_value) - 1:
                ...
                elif ripple_value[k] > 3.0:
                    loc.remove(location_peak[ripple_value.index(ripple_value[k])])
                elif ripple_value[k] < 0.4:
                    loc.remove(location_peak[ripple_value.index(ripple_value[k])])
        ...
        # print mark[-1] * 0.0000625
        # print length_o/16000 - 0.02
        # for i in range(0, len(mark), 2):  # Writing the result to a CSV File
        #     print mark[i] * 0.0000625, mark[i + 1] * 0.0000625

        if mark[0] != 0.0:
            text_file_1.write('%06.3f' % (0 * 0.0000625) + "\t" + '%06.3f' % (mark[0] * 0.0000625) + "\t" + "Non-Vowel" + "\n")
        ...
        for i in range(0, len(mark)-1):  # Writing the result to a CSV File
            if i%2 == 0:
                ...
                text_file_1.write(
                '%06.3f' % (mark[i] * 0.0000625) + "\t" + '%06.3f' % (mark[i + 1] * 0.0000625) + "\t" + "Non-Vowel" + "\n")


        if mark[-1] != length_o/16000 - 0.02:
            text_file_1.write('%06.3f' % (mark[-1] * 0.0000625) + "\t" + '%06.3f' % (length_o/16000 - 0.02) + "\t" + "Non-Vowel" + "\n")
...
file_name_template_1 = 'F:\Projects\Active Projects\Project Intern_IITB\Vowel Evaluation PE V5\\'
result_name = 'Vowel_Evaluation_V5_I8_Test_1.csv'

copyfile('C:\Users\Mahe\PycharmProjects\untitled\IIT-B Internship\\Vowel Evaluation PE V5.py',
         'F:\Projects\Active Projects\Project Intern_IITB\Vowel Evaluation PE V5\\' + result_name[:-4] + '.txt')
...
for j in only_audio:
    filename1, count1 = peak_elimination(j, window_dur=50, hop_dur=7, degree=10, threshold_smooth=120)  # Run Peak elimination algorithm
