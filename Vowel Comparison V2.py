...
def peak_elimination(audiofile, window_dur=30, hop_dur=5, ripple_lower_limit=0.4, ripple_upper_limit=1.6):
    try:
...
def convexhull(audiofile, window_dur=50, hop_dur=5, threshold=0.3):
    try:
        audio_file = audiofile
...
def results_write(sr_no, name, vpe, vpee, vch, vche, vfa, t):
    if vpe == 0 or vch == 0 or vfa == 0:
...
file_no = 1
results_vowel = open('F:\Projects\Active Projects\Project Intern_IITB\Vowel Comparison V2\Vowel_Count_V29.csv', 'a')  # The csv file where the results are saved
only_audio = glob.glob('F:\Projects\Active Projects\Project Intern_IITB\Vowel Comparison V2\Data 1\*.wav')  # Extract file name of all audio samples
only_text = glob.glob('F:\Projects\Active Projects\Project Intern_IITB\Vowel Comparison V2\Data 1\*.TextGrid')  # Extract file name of all force aligned TextGrid
results_vowel.write('File No' + ',' + 'Name' + ',' + 'Peak_Elimination' + ',' + 'Evaluation Script PE' + ',' + 'Convex Hull' + ',' + 'Evaluation Script CH' + ',' + 'Vowel Count FA' + ',' + 'Duration(In Seconds)' + ',' + 'Precision PE' + ',' + 'Recall PE' + ',' + 'Precision CH' + ',' + 'Recall CH' + 'Tag' + '\n')    # Column headers

for j in only_audio:
    filename1, count1 = peak_elimination(j, window_dur=30, hop_dur=5, ripple_lower_limit=0.4, ripple_upper_limit=1.6)  # Run Peak elimination algorithm
    ...
    filename2, count2 = convexhull(j, window_dur=50, hop_dur=5, threshold=0.3)  # Run the convex hull algorithm
    textgrid(str(filename2[:-4] + 'CH.csv'))  # Create TextGrid based on results of convex hull
    count_ch = evaluation(str(j[:-4] + '.TextGrid'), str(filename2[:-4] + 'CH_NEW.TextGrid'))  # Evaluate the TextGrid created and force aligned TextGrid
    ...
    results_write(file_no, j, count1, count_pe, count2, count_ch, vowel_count, time)  # Writing the results in the csv file.
    file_no += 1
