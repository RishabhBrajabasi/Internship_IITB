...
def segmentation(frame_energy, count =1):
    ...
    convex_hull.append(last_2)
    diff_1 = [0] * len(convex_hull)
    for j in range(len(convex_hull)):
        diff_1[j] = convex_hull[j] - frame_energy[j]
        if diff_1[j] < 0:
            diff_1[j] *= 0
    ...
    convex_hull.append(last_2)
    diff_2 = [0] * len(convex_hull)
    for j in range(len(convex_hull)):
        diff_2[j] = convex_hull[j] - frame_energy[j]
        if diff_2[j] < 0:
            diff_2[j] *= 0

    diff = [0] * len(convex_hull)
    for j in range(len(diff_1)):
        diff[j] = diff_1[j] + diff_2[j]
    ...
    if max(diff) > 0.1:
        ...
        if len(st_energy[st:bp]) > 0 and len(st_energy[bp:sp]) > 0:
            ...
            #
            print "h1"
            segmentation(st_energy[st:bp], count = count + 1)

            print "b1"
            segmentation(st_energy[bp:sp], count= count  + 1)
        else:
            print "h1"
            segmentation(st_energy[st:bp], count=count + 1)
