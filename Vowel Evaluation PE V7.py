...
def peak_elimination(audio, window_dur=50, hop_dur=7, degree=5, threshold_smooth=100):
    ...
    try:
        ...
        for i in range(no_frames):  # Calculating frame wise short term energy
            frame = data[
                    i * hop_size:i * hop_size + window_size] * window_type  # Multiplying each frame with a hamming window
        ...
        for p in range(len(st_energy)):
            if p == 0:  # First element
                ...
                if st_energy[p] > st_energy[p + 1] and st_energy[p] > st_energy[
                    ...
                    # ----------------------------------------------------------------------------------------------------------------------#
        threshold = 0.01 + 0.04 * (
        noise_energy + (sum(peak) / count_of_peaks))  # The threshold which eliminates minor peaks.
        # ----------------------------------------------------------------------------------------------------------------------#
        ...
        for k in range(len(location_peak)):
            ...
            if location_peak[k] == len(peak) - 1:  # If the peak is the last element of the short term energy curve
                ...
                ripple.append(location[
                                  q - 1])  # The location of the valley before the last peak is added, as there is no valley after it
                ripple_valley.append(location[
                                         q - 1])  # The location of the valley before the last peak is added, as there is no valley after it
                ...
                ripple_peak.append(location[q])  # The location of the peak is added
                ripple.append(location[
                                  q + 1])  # The location of the valley after the first peak is added, as there is no valley after it
                ripple_valley.append(location[
                                         q + 1])  # The location of the valley after the first peak is added, as there is no valley after it
        ...
        for k in range(len(ripple_value)):
            if k != len(ripple_value) - 1:
                if location_peak[ripple_value.index(ripple_value[k + 1])] - location_peak[
        ...
        if mark[-1] != length_o / 16000 - 0.02:
            text_file_1.write(
                '%06.3f' % (mark[-1] * 0.0000625) + "\t" + '%06.3f' % (length_o / 16000 - 0.02) + "\t" + " " + "\n")
...
result_name = 'Vowel_Evaluation_V6_Test_7_compromise.csv'
...
for j in only_audio:
    filename1, count1 = peak_elimination(j, window_dur=30, hop_dur=5, degree=7,
                                         threshold_smooth=120)  # Run Peak elimination algorithm
    ...
    vowel_count, time = fa_count(str(
        j[:-4] + '.TextGrid'))  # Count the number of vowels in the audio file according to the force aligned TextGrid
