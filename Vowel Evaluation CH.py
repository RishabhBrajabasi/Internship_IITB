...
file_no = 1
results_vowel = open('F:\Projects\Active Projects\Project Intern_IITB\Vowel Evaluation CH\Vowel_Evaluation_4_Modified_Algo.csv', 'a')  # The csv file where the results are saved
only_audio = glob.glob('F:\Projects\Active Projects\Project Intern_IITB\Vowel Evaluation CH\Data 1\*.wav')  # Extract file name of all audio samples
only_text = glob.glob('F:\Projects\Active Projects\Project Intern_IITB\Vowel Evaluation CH\Data 1\*.TextGrid')  # Extract file name of all force aligned TextGrid
results_vowel.write('File No' + ',' + 'Name' + ',' + 'Convex Hull' + ',' + 'Evaluation Script CH' + ',' + 'Vowel Count FA' + ',' + 'Duration(In Seconds)' + ',' + 'Precision CH' + ',' + 'Recall CH' + ',' + 'Tag' + '\n')    # Column headers
for j in only_audio:
    filename2, count2 = convexhull(j, window_dur=30, hop_dur=5, threshold=0.1)  # Run the convex hull algorithm
