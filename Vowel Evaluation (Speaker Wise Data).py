...
def savitzky_golay(y, window_size, order, deriv=0, rate=1):
    ...
    first_value = y[0] - np.abs(y[1:half_window + 1][::-1] - np.float64(y[0]))
    last_value = y[-1] + np.abs(y[-half_window - 1:-1][::-1] - np.float64(y[-1]))
    y = np.concatenate((first_value, y, last_value))
...
def peak_elimination(audio, window_dur=30, hop_dur=5, degree=3, threshold_smooth=120):
    new = ''
        ...
        for k in range(len(ripple_value)):
            if k != len(ripple_value) - 1:
                if location_peak[ripple_value.index(ripple_value[k+1])] - location_peak[ripple_value.index(ripple_value[k])] < 20:
                    if ripple_value[k] > 3.0 and ripple_value[k + 1] < 1.4 \
                            or ripple_value[k] > 1.02 and ripple_value[k + 1] < 0.3:
...
def textgrid(csvi):
    try:
        ...
        print 'TextGrid Creation Error : ',csvi
...
file_name_template_1 = "F:\Projects\Active Projects\Project Intern_IITB\Vowel Evaluation (Speaker Wise Data)\\"
result_name = 'Vowel_Evaluation_V5_Speaker_Based.csv'

copyfile('C:\Users\Mahe\PycharmProjects\untitled\IIT-B Internship\\Vowel Evaluation (Speaker Wise Data).py',
         file_name_template_1 + result_name[:-4] + '.txt')

directories=[d for d in os.listdir(file_name_template_1) if os.path.isdir(file_name_template_1)]
directories.pop(0)

directory = [''] * len(directories)
for d in range(len(directories)):
    directory[d] = file_name_template_1 + directories[d]

precision = []
recall = []
speaker_results = open(file_name_template_1 + result_name, 'w')  # The csv file where the results are saved
speaker_results.write('Speaker' + ',' + 'Precision' + ',' + 'Recall'  + ',' + 'No. of Audio files' + '\n')
for speaker in directory:
    file_no = 1
    results_vowel = open(speaker + '.csv', 'w')  # The csv file where the results are saved
    only_audio = glob.glob(speaker + '\\Data 1\*.wav')  # Extract file name of all audio samples
    only_text = glob.glob(speaker + '\\Data 1\*.TextGrid')  # Extract file name of all force aligned TextGrid
    ...
    for j in only_audio:
        ...
        file_no += 1
    try:
        x=str(speaker).find('Data')
        speaker_results.write(str(speaker[x+6:]) + ',' + str(sum(precision)/len(precision)) + ',' + str(sum(recall)/len(recall)) + ',' + str(len(precision)) + '\n')
    except:
        x = str(speaker).find('Data')
        speaker_results.write(str(speaker[x+6:]) + ',' + str(0) + ',' + str(0) + ',' + str(len(precision)) + '\n')
    precision[:] = []
    recall[:] = []
