...
# ----------------------------------------------------------------------------------------------------------------------#
def peak_elimination(audio, window_dur=50, hop_dur=7, degree=20, threshold_smooth=100):
    new = ''
    index = audio.find('Data 1')
    new = new + (audio[0: index+5])
    new += '2\\'
    new = new + (audio[index+7:])  # New path to store CSV file.
    text_file_1 = open(str(new[:-4]) + 'PE.csv', 'w')  # Opening CSV file to store results and to create TextGrid
    try:
        fs, data1 = wavfile.read(audio)  # Reading data from wav file in an array
        data1 = data1 / float(2 ** 15)  # Normalizing it to [-1,1] range from [-2^15,2^15]
        data = butter_bandpass_filter(data1, 300, 2500, fs, order=6)  # Butterworth bandpass filter of order 6.
        length_o = len(data)
        ...
        if len(st_energy) < threshold_smooth:
            ...
            st_energy = moving_average(st_energy, degree)
        # ----------------------------------------------------------------------------------------------------------------------#
            ...
            if k != len(ripple_value) - 1:
                if location_peak[ripple_value.index(ripple_value[k + 1])] - location_peak[ripple_value.index(ripple_value[k])] < 20:
                    if ripple_value[k] > 3.0 and ripple_value[k + 1] < 1.4 or ripple_value[k] > 1.02 and ripple_value[k + 1] < 0.3:
                        ...
                        if v1 >= v2:
                            the_list.append(location_peak[ripple_value.index(ripple_value[k + 1])])
                        else:
                            the_list.append(location_peak[ripple_value.index(ripple_value[k])])
            else:
        ...
        if sum(peak_threshold) == 0:
            mark.append(-1)
        if mark[0] == -1:
            text_file_1.write('%06.3f' % 0 + "\t" + '%06.3f' % ((length_o / 16000) - 0.02) + "\t" + " " + "\n")
        if mark[0] != 0:
...
cases = len(wd)*len(hd)*len(ts)*len(d)

results_opt = open('F:\Projects\Active Projects\Project Intern_IITB\Parameter Optimization PE V5\Vowel_opt_Window and Hop Test.csv', 'a')
if os.path.getsize('F:\Projects\Active Projects\Project Intern_IITB\Parameter Optimization PE V5\Vowel_opt_Window and Hop Test.csv') == 0:
    results_opt.write('File Name' + ',' + 'Window Duration' + ',' + 'Hop Duration' + ',' + 'Degree' + ',' + 'Smooth Threshold' + ',' + 'Precision' + ',' + 'Recall' + '\n')

file_name_template_1 = 'F:\Projects\Active Projects\Project Intern_IITB\Parameter Optimization PE V5\\'
    ...
    for hdd in hd:
        for tss in ts:
            for dd in d:
                ...
                for j in only_audio:
                    filename1, count1 = peak_elimination(j, window_dur=wdd, hop_dur=hdd,  degree=dd,threshold_smooth=tss)
                    ...
                    file_no += 1
                results_opt.write(str(result_name[file_count]) + ',' + str(wdd) + ',' + str(hdd) + ',' + str(dd) + ',' + str(tss) + ',' +
